...
public enum ZmartifyDeviceType {
	ENERGYMETERWATT("energyMeterWatt", "DecimalType", "Number", "%s", "state", "energyWatt",
			"Energy Meter in Watts"), 
	ENERGYMETERKVAH("energyMeterKVAH", "DecimalType", "Number", "%s", "state",
					"energyKVAH", "Energy Meter in KVAH"), 
	ENERGYMETERVOLT("energyMeterVolt", "DecimalType", "Number",
							"%s", "state", "energyVolt", "Energy Meter in Volt"), 
	ENERGYMETERAMP("energyMeterAmp",
									"DecimalType", "Number", "%s", "state", "energyAmp",
									"Energy Meter in Amp"), 
	ENERGYMETER("energyMeter", "DecimalType", "Number", "%s",
											"state", "energy", "Energy Meter"), 
	ALARM("alarm", "OnOffType", "Number",
													"%s", "state", "alarm", "Alarm"), 
	BATTERYLEVEL("batteryLevel",
															"PercentType", "Number", "%s", "state", "battery",
															"Battery level"), 
	BUTTON("button", "OnOffType", "Switch",
																	"%s", "state", "button", "Button"), 
	CONTACTSENSOR(
																			"contactSensor", "OpenClosedType",
																			"Contact", "%s", "state", "contactSensor",
																			"Contact Sensor"), LOCK("lock",
																					"DecimalType", "Number", "%s",
																					"state", "lock",
																					"Lock"), 
	NOTIFICATIONMESSAGE(
																							"notificationMessage",
	...
	SWITCHLEVEL(
																																	...
																																	"state",
																																	"switchLevel",
																																	"Switch Level"), THERMOSTATHEATINGSETPOINT(
																																			...
																																			"state",
																																			"setpoint",
																																			"Thermostat Heating Setpoint"), THERMOSTATMODE(
																																					"thermostatMode",
																																					"DecimalType",
																																					"Number",
																																					"%s",
																																					"state",
																																					"themostatMode",
																																					"Thermostat Mode"), THERMOSTATOPERATINGSTATE(
																																							...
																																							"state",
																																							"thermostatOperatingState",
																																							"Thermostat Operating State"), THERMOSTATSETPOINT(
																																									"thermostatSetpoint",
																																									"DecimalType",
																																									"Number",
																																									"%s",
																																													...
																																													"Relay Switch"), SLEEPSENSOR(
																																															...
																																															"Sleep Sensor"), TAMPERALERT(
																																																	...
																																																	"Tamper Alert"), TEMPERATURE(
																																																			...
																																																			"state",
																																																			"temperature",
																																																			"Temperature Measurement"), FLOORTEMPERATURE(
																																																					"floorTemperature",
																																																					"DecimalType",
																																																					"Number",
																																																					"%s",
																																																					"state",
																																																					"floorTemperature",
																																																					"Floor temperature measurement"), AIRTEMPERATURE(
																																																							"airTemperature",
																																																							"DecimalType",
																																																							"Number",
																																																							"%s",
																																																							"state",
																																																							"airTemperature",
																																																							"Air temperature measurement"), OUTLET(
																																																									"outlet",
																																																									"OnOffType",
																																																									"Switch",
																																																									"%s",
	...
	private String appendRequired(String required, String append) {
		if (required == null) {
			return "\"" + append + "\"";
		} else {
			return required + ", \"" + append + "\"";
		}
	}
	...
	private void buildDefaultMapping() {
		...
		addProperty("eventCount", null, "number", (int) -1, "$state.eventCount+1");
		switch (applicationInterfaceProperty) {
		case "temperature":
		case "floorTemperature":
		case "airTemperature":
		case "thermostatSetPoint":
			addProperty(applicationInterfaceProperty, "C", "number", (Number) 0, "%s");
			addProperty(applicationInterfaceProperty, "F", "number", (Number) 0, "%s * 1.8 + 32");
			addProperty(applicationInterfaceProperty, "isLow", "boolean", (Boolean) false,
					"%s < $state." + applicationInterfaceProperty + ".lowest");
			addProperty(applicationInterfaceProperty, "isHigh", "boolean", (Boolean) false,
					"%s > $state." + applicationInterfaceProperty + ".highest");
			addProperty(applicationInterfaceProperty, "lowest", "number", (Number) 100,
					"(%s < $state." + applicationInterfaceProperty + ".lowest) ? %s : $state."
							+ applicationInterfaceProperty + ".lowest");
			addProperty(applicationInterfaceProperty, "highest", "number", (Number) 0,
	...
	public JsonObject getAPIJson() {
		apiBuilder.remove("applicationInterfaceId");
		return apiBuilder;
	}
